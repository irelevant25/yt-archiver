...
| Setup hijacking after install | `setup.php` 404 once `installed`; while in progress it requires the 48-hex `setup_token`; step 1 form has a double-submit CSRF cookie | setup.php |
...
| Secrets on disk | `DATA_DIR/config.php` 0600, sessions 0700, `/data` never served; SQL errors are not sent to the browser | entrypoint.sh, api.php |
